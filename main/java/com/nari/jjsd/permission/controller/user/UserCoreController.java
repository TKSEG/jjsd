...
import com.nari.jjsd.core.manager.UserManager;
...
/**
 * Created by yangbo on 2018/12/7.
 */
@Controller
@RequestMapping("user")
public class UserCoreController extends BaseController {
    ...
    public ModelAndView index(HttpServletRequest request) {
        ...
        modelAndView.addObject("_time", System.currentTimeMillis());
        modelAndView.addObject("NOW_YEAY", Constant.NOW_YEAY);//今年
        modelAndView.addObject("_v", Constant.VERSION);//版本号，重启的时间
        modelAndView.addObject("basePath", request.getContextPath());//base目录。model.put(Constant.CONTEXT_PATH, request.getContextPath());
        return modelAndView;
    }
    ...
    public Object update(User user, String rids) {
        //如果密码为空表示不修改密码
        if (com.nari.jjsd.util.commom.StringUtils.isBlank(user.getPswd())) {
            user.setPswd(null);
        }
